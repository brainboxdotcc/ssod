...
dpp::task<void> delete_live_player(const dpp::interaction_create_t& event) {
	...
	co_await db::co_query("DELETE FROM quest_progress WHERE user_id = ?", { event.command.usr.id });
	co_await db::co_query("DELETE FROM character_bio WHERE user_id = ?", { event.command.usr.id });
	co_return;
}
